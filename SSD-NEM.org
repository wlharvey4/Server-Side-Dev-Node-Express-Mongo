...
- =C-c C-v s= :: runs the command ~org-babel-execute-subtree~, which calls
                 ‘org-babel-execute-src-block’ on every source block in the
                 current subtree.  In this subtree, all source blocks are BASH
                 shell commands, which produce no output because =:results
                 silent= is set in the =header-args= in the =PROPERTIES= of the
                 outline heading.

- =C-c C-v t= :: runs the command ~org-babel-tangle~ in the subtree (in the
                 narrowed subtree)
...
- Now, copy the ~dishRouter.js~, ~promoRouter.js~ and ~leaderRouter.js~ from
  ...
  *NOTE*: Use Org ~tangle~ and ~noweb~ to add the source code for the custom
  routers from the prior week into the ~routes~ folder.

  #+name:node-express-generator-noweb-tangle-routers
  #+begin_src js :noweb yes :tangle conFusionServer/routes/dishRouter.js
  <<node-express-routers-dishRouter.js>>
  <<node-express-routers-promoRouter.js>>
  <<node-express-routers-leaderRouter.js>>
  #+end_src
...
- Furthermore, copy the ~index.html~ and ~aboutus.html~ file from the
  ...
  #+name:node-express-generator-add-noweb-tangle-index.html
  #+header: :eval no
